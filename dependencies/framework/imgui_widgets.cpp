...
#include <map>
#include <vector>
#include "C:\Users\rootste\source\repos\demon_services\dependencies\framework\backend\imgui_settings.h"
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, hovered ? text_color[0] : text_color[1], anim_speed);
    it_anim->second.frame_color = ImLerp(it_anim->second.frame_color, hovered ? main_color : scroll_bg_col, anim_speed);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, scroll_bg_col, style.FrameRounding);

    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(it_anim->second.frame_color), style.FrameRounding, 0, 1.5f);

    window->DrawList->AddText(center_text(bb.Min, bb.Max, label), text_color[0], label);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max, scroll_bg_col, 360.f);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
...
bool ImGui::Keybind(const char* label, int* key)
{
    ...
    if (label_str.find("##keybind") == std::string::npos)
        window->DrawList->AddText(ImVec2(total_bb.Min.x, center_text(total_bb.Min, total_bb.Max, label).y), text_color[0], label);

    if (*key > 0 && !it_anim->second.mode && *key != 27)
    {
        ...
        window->DrawList->AddText(ImVec2(total_bb.Max.x -  it_anim->second.key_size.x, center_text(total_bb.Min, total_bb.Max, it_anim->second.key_name).y), GetColorU32(it_anim->second.key_color), it_anim->second.key_name);
    }
    ...
    {
        PushFont(icon_font);
        it_anim->second.key_size = ImLerp(it_anim->second.key_size, CalcTextSize("0"), anim_speed);
        window->DrawList->AddText(ImVec2(total_bb.Max.x  - it_anim->second.key_size.x, center_text(total_bb.Min, total_bb.Max, "0").y), GetColorU32(it_anim->second.key_color), "0");
        PopFont();
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(total_bb.Min, total_bb.Min + ImVec2(20, 20));
    ItemSize(total_bb, style.FramePadding.y);
    ItemAdd(total_bb, id);
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v ? text_color[0] : text_color[1], anim_speed);
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v ? main_color : ImColor(main_color.Value.x, main_color.Value.y, main_color.Value.z, 0.f), anim_speed);
    it_anim->second.mark_rotate = ImLerp(it_anim->second.mark_rotate, *v ? IM_PI / 2 : IM_PI * 2, anim_speed);
    it_anim->second.mark_color = ImLerp(it_anim->second.mark_color, *v ? ImColor(0.1f, 0.1f, 0.2f, 1.f) : ImColor(0.1f, 0.1f, 0.2f, 0.f), anim_speed);
    it_anim->second.mark_size = ImLerp(it_anim->second.mark_size, *v ? 9.f : 0.f, anim_speed);

    window->DrawList->AddText(ImVec2(total_bb.Min.x + 30, center_text(total_bb.Min, total_bb.Max, label).y), GetColorU32(it_anim->second.text_color), label);

    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, background_color, style.FrameRounding);
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.rect_color), style.FrameRounding);

    PushFont(small_icon_font); ImRotateStart();

    RenderCheckMark(window->DrawList, check_bb.GetCenter() - ImVec2(it_anim->second.mark_size / 2, it_anim->second.mark_size / 2), GetColorU32(it_anim->second.mark_color), it_anim->second.mark_size);
...
bool ImGui::SubTab(const char* label, int* v, int number)
{
    ...
    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(164, 40));
    ...
    it_anim->second.icon_color = ImLerp(it_anim->second.icon_color, *v == number ? text_color[0] : text_color[1], anim_speed);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v == number ? text_color[0] : text_color[1], anim_speed);
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v == number ? ImColor(main_color.Value.x, main_color.Value.y, main_color.Value.z, 0.5f) : ImColor(main_color.Value.x, main_color.Value.y, main_color.Value.z, 0.f), anim_speed);

    PushClipRect(total_bb.Min, total_bb.Max, true);
    window->DrawList->AddRectFilledMultiColor(total_bb.Min, total_bb.Max + ImVec2(50, 0), ImColor(main_color.Value.x, main_color.Value.y, main_color.Value.z, 0.f), GetColorU32(it_anim->second.rect_color), GetColorU32(it_anim->second.rect_color), ImColor(main_color.Value.x, main_color.Value.y, main_color.Value.z, 0.f));
    PopClipRect();

    window->DrawList->AddRectFilled(ImVec2(total_bb.Max.x - 2, total_bb.Min.y), total_bb.Max, GetColorU32(it_anim->second.rect_color));

    window->DrawList->AddText(center_text(total_bb.Min, total_bb.Max, label), GetColorU32(it_anim->second.text_color), label);
...
bool ImGui::Tab(const char* label, const char* icon, int* v, int number)
{
    ...
    PushFont(tab_font);
    const ImRect total_bb(pos, pos + ImVec2(46 + CalcTextSize(label).x, 30));
    PopFont();
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v == number ? text_color[0] : text_color[1], anim_speed);
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v == number ? background_color : GetColorWithAlpha(background_color, 0.f), anim_speed);

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(it_anim->second.rect_color), 5.f);

    PushFont(tab_font);
    window->DrawList->AddText(ImVec2(total_bb.Min.x + 34.f, center_text(total_bb.Min, total_bb.Max, label).y), GetColorU32(it_anim->second.text_color), label);
    PopFont();

    PushFont(big_icon_font);
    window->DrawList->AddText(ImVec2(total_bb.Min.x + 12.f, center_text(total_bb.Min, total_bb.Max, icon).y), GetColorU32(it_anim->second.icon_color), icon);
    PopFont();
    return pressed;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int value)
{
    ...
    const ImRect total_bb(pos, pos + frame_size + ImVec2(0, 11));
    ItemSize(total_bb, style.FramePadding.y);
    ItemAdd(total_bb, id);
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, it_anim->second.opened_combo ? text_color[0] : text_color[1], anim_speed);
    it_anim->second.frame_color = ImLerp(it_anim->second.frame_color, it_anim->second.opened_combo ? ImColor(37, 33, 49, 255) : ImColor(37, 33, 49, 0), anim_speed);
    it_anim->second.text_alpha = ImLerp(it_anim->second.text_alpha, it_anim->second.opened_combo ? 255 : 0, anim_speed);
    it_anim->second.icon_rotate = ImLerp(it_anim->second.icon_rotate, it_anim->second.opened_combo ? IM_PI * 1.5f : IM_PI / 2, anim_speed);

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, background_color, style.FrameRounding);

    ImRotateStart();
    PushFont(arrow_icons);
    window->DrawList->AddText(total_bb.Max - ImVec2(20.f, 15.5f) - CalcTextSize("1") / 2, text_color[1], "1");
    PopFont();

    ImRotateEnd(it_anim->second.icon_rotate);

    ImVec2 label_size = CalcTextSize(label);

    window->DrawList->AddText(ImVec2(total_bb.Min.x + 10, center_text(total_bb.Min, total_bb.Max, label).y), GetColorU32(it_anim->second.text_color), label);

    std::string value_str = "*" + std::to_string(value);
    PushFont(small_font);
    window->DrawList->AddText(ImVec2(total_bb.Min.x + 10, center_text(total_bb.Min, total_bb.Max, value_str.c_str()).y) + ImVec2(label_size.x + 1, -3), text_color[1], value_str.c_str());
    PopFont();
    ...
    if (!it_anim->second.opened_combo && it_anim->second.text_alpha < 5)
        return false;

    ImGui::SetNextWindowPos(ImVec2(total_bb.Min.x, total_bb.Max.y + 3));

    ImGui::SetNextWindowSize(ImVec2(total_bb.GetWidth(), 0));

    PushStyleVar(ImGuiStyleVar_WindowRounding, style.FrameRounding);
    PushStyleColor(ImGuiCol_WindowBg, ImColorToImVec4(it_anim->second.frame_color));

    main_color = ImColor(163, 149, 216, it_anim->second.text_alpha);
    text_color[1] = ImColor(86, 82, 97, it_anim->second.text_alpha);
...
void ImGui::EndCombo()
{
    ...
    main_color = ImColor(163, 149, 216, 255);
    text_color[1] = ImColor(86, 82, 97, 255);
    End();
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 item_size = frame_size + ImVec2(0, 9);

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + item_size);

    const ImRect slider_bb(total_bb.Min + ImVec2(0, 21), total_bb.Max - ImVec2(0, 3));
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, hovered ? text_color[0] : text_color[1], anim_speed * 2);

    window->DrawList->AddRectFilled(slider_bb.Min, slider_bb.Max, background_color, 30);

    window->DrawList->AddRectFilled(slider_bb.Min - ImVec2(0, 2), ImVec2(slider_bb.Min.x + it_anim->second.grab_bb_center_offset.x, slider_bb.Max.y + 2), main_color, 30);

    window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + it_anim->second.grab_bb_center_offset.x - 5, slider_bb.GetCenter().y), 2, GetColorWithAlpha(background_color, 1.f));
    ...
    window->DrawList->AddText(total_bb.Min - ImVec2(0, 2), text_color[0], label);
    window->DrawList->AddText(ImVec2(slider_bb.Max.x - CalcTextSize(value_buf).x, total_bb.Min.y) - ImVec2(0, 2), text_color[1], value_buf);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        RenderNavHighlight(frame_bb, id);
        ImGui::GetForegroundDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, stroke_color, style.FrameRounding);
    }
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                       ImGui::GetForegroundDrawList()->AddRectFilled(rect.Min, rect.Max, bg_color);
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            ImGui::GetForegroundDrawList()->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
           ImGui::GetForegroundDrawList()->AddText(g.Font, g.FontSize, draw_pos, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        ImGui::SetCursorScreenPos(total_bb.Max - ImVec2(40, 30));
        ...
        {
            ImGui::Begin("picker_edit", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground);
            if (g.CurrentWindow->BeginCount == 1)
            {
                ImRect picker_bb(GetWindowPos(), GetWindowPos() + ImGui::GetWindowSize());

                if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) && !ImGui::IsMouseHoveringRect(picker_bb.Min, picker_bb.Max, false))
                    it_anim->second.picker_active = false;

                ImGui::GetForegroundDrawList()->AddRectFilled(ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImGui::GetWindowSize(), winbg_color, 10.f);
                ImGui::GetForegroundDrawList()->AddRectFilled(ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImGui::GetWindowSize(), ImColor(1.f, 1.f, 1.f, 0.05f), 10.f);
                ImGui::GetForegroundDrawList()->AddRect(ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImGui::GetWindowSize(), stroke_color, 10.f);
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        window->DrawList->AddText(ImVec2(total_bb.Min.x + 20.f, center_text(total_bb.Min, total_bb.Max, label).y), GetColorU32(it_anim->second.color), label);
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    {
        // Render SV Square
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), child_color, col_white, hue_color32, hue_color32, col_white, style.FrameRounding, ImDrawFlags_RoundCornersAll);
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), child_color, 0, 0, col_black, col_black, style.FrameRounding, ImDrawFlags_RoundCornersAll);
        ...
        // Render Hue Bar
        for (int i = 0; i < 6; ++i)
            draw_list->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width,
                picker_pos.y + (i + 1) * (sv_picker_size / 6)), child_color, col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1], 10.f,
                i == 0 ? ImDrawFlags_RoundCornersTop : i == 5 ? ImDrawFlags_RoundCornersBottom : ImDrawFlags_RoundCornersNone);
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilledMultiColorRounded(bar1_bb.Min, bar1_bb.Max, child_color, user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, 10.f, ImDrawFlags_RoundCornersAll);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4 & col, ImGuiColorEditFlags flags, const ImVec2 & size_arg)
{
    ...
    window->DrawList->AddRectFilled(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), background_color, 360.f);

    window->DrawList->AddRectFilled(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), GetColorU32(col_rgb), 360.f);

    window->DrawList->AddRect(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), stroke_color, 360.f);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? main_color : text_color[1], anim_speed);

    window->DrawList->AddText(text_min, GetColorU32(it_anim->second.text_color), label);
